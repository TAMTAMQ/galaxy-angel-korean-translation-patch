...
존댓말·반말을 그대로 따르며, 호칭은 `씨/군/쨩/님/나리` 고정 대응을 적용한다.
...
번역 문장을 고치기 전에 `system_review_20260910.md` 를 읽는다. 바이트 상한, 44열 재래핑,
선택지 고정 슬롯, 폰트 글리프 잔여 용량 같은 **어기면 빌드가 중단되는 제약**과 해당 항목
목록이 들어 있다. 위에 적힌 "안전 한도 고유 한글 2,045자"는 필터 적용 전 값이며 실제 용량은
1,352자다(2026-09-10 사용 1,220자, 여유 132자).
