...
def parse_pes_header(data: bytes, offset: int, packet_size: int) -> tuple[int, int | None, int | None]:
    ...
    if data[offset + 6] & 0xC0 != 0x80:
        # MPEG-1 style PES is not expected for this title. Keep a conservative payload start.
        return offset + 6, None, None
    flags = data[offset + 7]
...
def parse_program_stream(path: Path) -> tuple[bytes, list[Packet]]:
    ...
    while pos < n:
        ...
        if sid == 0xB9:
            ...
            # MPEG-2 pack header. Low three bits of byte 13 are stuffing length.
...
def main() -> int:
    ...
    # Reconstruct the elementary video stream and retain the PES-to-ES mapping.
    es = bytearray()
